...
const TodoCards = ({ taskObj, index, deleteTodo,updateListArray }) => {
    ...
    return (
        <>
            <div className=" mb-3 card-ver" style={{ "borderBottom": `4px solid ${colors[index%6].primaryColor}` }}>
            <h5 tag="h5" className=" name" style={{"background": colors[index%6].secondaryColor}}>{taskObj.Name}</h5>
                <CardBody style={{"position": "relative", "height": "100%", "paddingLeft": "20px", "paddingTop": "10px"}}>
                    <CardText tag="p" className=" desc">{taskObj.Description}</CardText>
                    <div className="icons">
                         <i
                            className="far fa-edit edit"
                            style={{ color: colors[index % 6].primaryColor, cursor: "pointer" }}
